...
public class ModbusClient 
{
	...
	protected String ipAddress = "192.168.0.121";
	protected int port = 502;
//	protected String ipAddress = "192.168.101.100";
...
//	private byte unitIdentifier = 3;
	...
	private int connectTimeout = 1000*40;
    ...
    public void setTCPConfiguration (String tcpIP, int port)
    {
    	
    	this.ipAddress = tcpIP;
    	this.port = port;
    }
    
    public void setConfiguration(String comPort, int baudRate, String parity, String stopbits,boolean isbytesHigh,String deviceid)
    {
    	...
    	else if (stopbits.equalsIgnoreCase("1"))
    	{
    		this.stopBits = StopBits.One;
    	}
    	else if (stopbits.equalsIgnoreCase("2"))
    	{
...
//	}
	...
	public void Connect() throws UnknownHostException, IOException
	{
		...
		if (this.serialflag)
		{
		    ...
		    try {
				...
				serialPort.setComPortParameters(this.baudrate,
									8,
	...
	public void Connect(String ipAddress, int port) throws UnknownHostException, IOException
	{
        ...
        inStream = tcpClientSocket.getInputStream();
      //inStream = new DataInputStream(tcpClientSocket.getInputStream());
		System.out.println("InPutStream::::"+inStream);
		System.out.println("OutPutStream::::"+outStream);
		System.out.println("Open TCP-Socket, IP-Address: " + ipAddress + ", Port: " + port);
    ...
    */
     ...
     * @throws UnknownHostException 
     */
    public boolean[] ReadDiscreteInputs(int startingAddress, int quantity, String mqttBrokerAddress) throws MqttPersistenceException, MqttException, UnknownHostException, SocketException, ModbusException, IOException
    {
                ...
                UnknownHostException, SocketException, IOException 
	{
        ...
        {        	
            ...
            while (receivedUnitIdentifier != this.unitIdentifier & !((DateTime.getDateTimeTicks() - dateTimeSend) > 10000 * this.connectTimeout))
            {
                	...
                	receivedLen = serialPort.readBytes(serialdata, expectedlength); 
     ...
     * @throws UnknownHostException 
     */
    public boolean[] ReadCoils(int startingAddress, int quantity, String mqttBrokerAddress) throws MqttPersistenceException, MqttException, UnknownHostException, SocketException, ModbusException, IOException 
    {
     ...
     * @throws UnknownHostException 
     */
    public int[] ReadHoldingRegisters(int startingAddress, int quantity, String mqttBrokerAddress) throws MqttPersistenceException, MqttException, UnknownHostException, SocketException, ModbusException, IOException
    {
                ...
                UnknownHostException, SocketException, IOException 
	{
		...
		byte[] data = new byte[]
            ...
            if (this.serialflag)
            {
                ...
                System.out.println("bytes::"+Arrays.toString(crc)); 
            ...
            {          
            	...
            	java.lang.System.arraycopy(data, 6,serialdata,0,8);
        		//serialPort.purgePort(SerialPort.PURGE_RXCLEAR);
            	System.out.println(Arrays.toString(serialdata));
        		int writeLen = serialPort.writeBytes(serialdata, serialdata.length);
        		System.out.println(" Bytes writeen successfully"+writeLen +"data:"+Arrays.toString(serialdata));
        		if (debug) StoreLogData.getInstance().Store("Send Serial-Data: "+ Arrays.toString(serialdata));
               long dateTimeSend = DateTime.getDateTimeTicks();
               byte receivedUnitIdentifier = (byte)0xFF;
               serialdata = new byte[256];
               ...
               {
            	   System.out.println("not received identifier yet");
            	   
            	 if (  ( (DateTime.getDateTimeTicks() - dateTimeSend) > (1000000 * this.connectTimeout) ))
            	 {
            		 System.out.println(" Timed out no data in the socket ..");
            		 break;
            	 }
                ...
                if (serialdata != null)
               {
                   data = new byte[262]; 
                   System.arraycopy(serialdata, 0, data, 6, serialdata.length);
                   System.out.println("Receive ModbusRTU-Data: " + Arrays.toString(data));
                   if (debug) StoreLogData.getInstance().Store("Receive ModbusRTU-Data: " + Arrays.toString(data));
               }
                for (int i = 0; i < quantity; i++)
                {
                    ...
                    System.out.println("res::"+response[i]); 
		...
		if (tcpClientSocket.isConnected() | udpFlag)
		{
			...
			else
			{
				...
				{
//				int numberOfBytes = inStream.read(data, 0, data.length);
				int numberOfBytes = inStream.read(data,0,13);
				System.out.println("DataLength"+numberOfBytes);
				if (receiveDataChangedListener.size() > 0)
				{
					receiveData = new byte[numberOfBytes];
					System.arraycopy(data, 0, receiveData, 0, numberOfBytes);
     ...
     * @throws UnknownHostException 
     */
    public int[] ReadInputRegisters(int startingAddress, int quantity, String mqttBrokerAddress) throws MqttPersistenceException, MqttException, UnknownHostException, SocketException, ModbusException, IOException
    {
                ...
                UnknownHostException, SocketException, IOException 
	{
        ...
        {        
           ...
           while (receivedUnitIdentifier != this.unitIdentifier & !((DateTime.getDateTimeTicks() - dateTimeSend) > 10000 * this.connectTimeout))
           {
        	   receviedLen = serialPort.readBytes(serialdata, expectedlength); 
                ...
                UnknownHostException, SocketException, IOException 
    {
        ...
        {             
    		...
    		//serialPort.purgePort(SerialPort.PURGE_RXCLEAR);
    		//serialPort.writeBytes(serialdata, serialdata.length);
            int sendLength=serialPort.writeBytes(serialdata, serialdata.length);
    		
    		if (sendLength != serialdata.length)
    		{
    			System.out.println ("Socket writing failed.. Retrying..");
    			
    			
    			try
    			{
    				   while (serialPort.bytesAwaitingWrite() != 0 && sendLength == -1)
    				   {
     				         Thread.sleep(100);
     			    		...
     			    		sendLength=serialPort.writeBytes(serialdata, serialdata.length);
    				   }
    				   
    	    		System.out.println ("Socket writing successfull..bytes Written:"+  sendLength);
           ...
           int receivedLen = 0;
           while (receivedUnitIdentifier != this.unitIdentifier & !((DateTime.getDateTimeTicks() - dateTimeSend) > 100000* this.connectTimeout))
           {          
        	   receivedLen = serialPort.readBytes(serialdata, expectedlength); 
                ...
                UnknownHostException, SocketException, IOException 
    {
        ...
        this.startingAddress = toByteArray(startingAddress-40001);
...

    {
		...
		if (udpFlag)
		{
		    ...
		    clientSocket.send(sendPacket);
		    System.out.println("Send Packet"+sendPacket.getData().toString());
		    data = new byte[256];
		    DatagramPacket receivePacket = new DatagramPacket(data, data.length);
		    clientSocket.receive(receivePacket);
		    String received=new String(receivePacket.getData(),0,receivePacket.getLength());
		    System.out.println("Recieve Packet"+received);
		    clientSocket.close();
		    data = receivePacket.getData();
		}
		else
		{
    		...
    		System.out.println("Send ModbusTCP-Data:::"+Arrays.toString(data));
			...
			try 
			{
			    ...
			    // numberOfBytes = inStream.read(data, 0, 13);
			     
				if(bytesAvailable>0)
				{
			    numberOfBytes = inStream.read(data, 0, data.length);
				System.out.println("DataLength"+numberOfBytes);
				//if (receiveDataChangedListener.size() > 0)
				{
					receiveData = new byte[numberOfBytes];
					...
					/*for (ReceiveDataChangedListener hl : receiveDataChangedListener)
						hl.ReceiveDataChanged();
					if (debug) StoreLogData.getInstance().Store("Receive ModbusTCP-Data: " + Arrays.toString(data));*/
					System.out.println("Receive ModbusTCP-Data: " + Arrays.toString(data));
				}
				} 
